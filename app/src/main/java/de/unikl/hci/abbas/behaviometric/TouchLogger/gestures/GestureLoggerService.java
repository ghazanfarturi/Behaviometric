...
import com.dell.research.continuousauthentication.utils.GestureLoggerData;
...
import de.unikl.hci.abbas.behaviometric.TouchLogger.activities.GestureLoggerActivity;

public class GestureLoggerService extends Service {
    ...
    // Put the device polling operation on its own thread with a boolean
    // sentinel
    ...
    private LinkedList<GestureLoggerData> holdingBuffer = null;

    private LinkedList<GestureLoggerData> logBuffer = null;
    ...
    private GestureDataWriter dataWriter = null;
    ...
    public void onCreate() {
        ...
        try {
            ...
            holdingBuffer = new LinkedList<GestureLoggerData>();
            logBuffer = new LinkedList<GestureLoggerData>();
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (setupSuccess) {
            boolean startService = intent.getBooleanExtra(GestureLoggerService.EXTRA_START_SERVICE, false);

            if (startService) {
                try {
                    startLogging();
                    startForeground(Constants.NOTIFICATION_ID, buildNotification().build());
    ...
    private void startLogging() throws IOException, InterruptedException {
        nextLogWrite = System.currentTimeMillis() + Constants.LOG_FLUSH_TIMEOUT_MILLIS;
    ...
    private void logData(NativeDeviceData data) {
        ...
        GestureLoggerData loggerData = new GestureLoggerData(data, appName, rotation, roll, pitch, yaw, accX, accY, accZ);
        ...
        if (logBuffer.size() >= Constants.LOG_BUFFER_MAX_SIZE_LINES) {
            LinkedList<GestureLoggerData> writeBuffer = logBuffer;
     ...
     * @param logBuffer
     */
    private void writeTimestampNamedLogFile(final LinkedList<GestureLoggerData> logBuffer) {
        if (!logBuffer.isEmpty()) {
            String timeTag = Constants.DATE_FORMAT.format(Calendar.getInstance().getTime());
            StringBuffer logLines = new StringBuffer();

            dataWriter.writeCompressedLoggerData(logBuffer, timeTag + ".gz");
            nextLogWrite = System.currentTimeMillis() + Constants.LOG_FLUSH_TIMEOUT_MILLIS;
        }
    }
    ...
    private NotificationCompat.Builder buildNotification() {
        ...
        Intent openIntent = new Intent(this, GestureLoggerActivity.class);
